...
const FluidAnimation = () => {
  ...
  // Main effect: setup canvas, particles, and animation loop
  useEffect(() => {
    ...
    // store contexts for theme effect
    ctxRef.current = ctx;
    ...
    function cell(x, y, res) {
      ...
      this.col = 0; this.row = 0; this.xv = 0; this.yv = 0; this.pressure = 0;
    }
    function particle(x, y) { this.x = this.px = x; this.y = this.py = y; this.xv = this.yv = 0; }

    function init() {
      canvas.width = canvas_width; canvas.height = canvas_height;
      offscreenCanvas.width = canvas_width; offscreenCanvas.height = canvas_height;

      particles = Array.from({ length: speck_count }, () => new particle(Math.random() * canvas_width, Math.random() * canvas_height));
      ...
      vec_cells = Array.from({ length: num_cols }, (_, col) =>
        Array.from({ length: num_rows }, (_, row) => {
          ...
          cell_data.col = col; cell_data.row = row; return cell_data;
        })
      );
    ...
    function update_particle() {
      ...
      particles.forEach(p => {
        if (p.x >= 0 && p.x < canvas_width && p.y >= 0 && p.y < canvas_height) {
          ...
          p.x += p.xv; p.y += p.yv;
          offscreenCtx.moveTo(p.x, p.y); offscreenCtx.lineTo(p.px, p.py);
          p.px = p.x; p.py = p.y;
        } else {
    ...
    function change_cell_velocity(cell_data, mvelX, mvelY, pen_size) {
      const dx = cell_data.x - mouse.x; const dy = cell_data.y - mouse.y;
      let dist = Math.sqrt(dy * dy + dx * dx);
      if (dist < pen_size) {
        if (dist < 4) dist = pen_size;
        const power = pen_size / dist;
        cell_data.xv += mvelX * power; cell_data.yv += mvelY * power;
      }
    }

    function update_pressure(cell_data) {
      const pressure_x = cell_data.up_left.xv * 0.5 + cell_data.left.xv + cell_data.down_left.xv * 0.5 - cell_data.up_right.xv * 0.5 - cell_data.right.xv - cell_data.down_right.xv * 0.5;
      const pressure_y = cell_data.up_left.yv * 0.5 + cell_data.up.yv + cell_data.up_right.yv * 0.5 - cell_data.down_left.yv * 0.5 - cell_data.down.yv - cell_data.down_right.yv * 0.5;
    ...
    function update_velocity(cell_data) {
      ...
      cell_data.xv *= 0.99; cell_data.yv *= 0.99;
    }

    function draw() {
      const mouse_xv = mouse.x - mouse.px; const mouse_yv = mouse.y - mouse.py;
      vec_cells.forEach(cell_datas => { cell_datas.forEach(cell_data => { if (mouse.down) change_cell_velocity(cell_data, mouse_xv, mouse_yv, pen_size); update_pressure(cell_data); }); });
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      update_particle(); ctx.drawImage(offscreenCanvas, 0, 0);
      vec_cells.forEach(cell_datas => { cell_datas.forEach(update_velocity); });
      mouse.px = mouse.x; mouse.py = mouse.y;
      rafId = requestAnimationFrame(draw);
    }

    function mouse_down_handler(e) { e.preventDefault(); mouse.down = true; }
    function mouse_up_handler() { mouse.down = false; }
    function touch_start_handler(e) { e.preventDefault(); const rect = canvas.getBoundingClientRect(); mouse.x = mouse.px = e.touches[0].pageX - rect.left; mouse.y = mouse.py = e.touches[0].pageY - rect.top; mouse.down = true; }
    function touch_end_handler(e) { if (!e.touches) mouse.down = false; }
    function mouse_move_handler(e) { e.preventDefault(); const rect = canvas.getBoundingClientRect(); mouse.px = mouse.x; mouse.py = mouse.y; mouse.x = e.clientX - rect.left; mouse.y = e.clientY - rect.top; }
    function touch_move_handler(e) { e.preventDefault(); const rect = canvas.getBoundingClientRect(); mouse.px = mouse.x; mouse.py = mouse.y; mouse.x = e.touches[0].pageX - rect.left; mouse.y = e.touches[0].pageY - rect.top; }
  ...
  // Theme watcher: update strokeStyle whenever theme (or CSS variable) changes
  useEffect(() => {
    try {
      ...
      const fallback = theme === 'dark' ? '#fff' : '#111';
      if (ctxRef.current) ctxRef.current.strokeStyle = fallback;
      if (offscreenCtxRef.current) offscreenCtxRef.current.strokeStyle = fallback;
    }
  }, [theme]);
